...
# 4. Создать JWT secret
kubectl create namespace finwise
...
3. **Docker образы** собраны и доступны
   ...
   **Вариант A: Автоматическая сборка (Рекомендуется)**
   ```bash
   # Использовать скрипт для сборки всех образов
   ./k8s/build-images.sh
   ```
...
├── build-images.sh                    # Скрипт сборки Docker образов
├── deploy.sh                          # Скрипт автоматического деплоя
├── undeploy.sh                        # Скрипт удаления ресурсов
...
└── jobs/                              # Одноразовые задачи (миграции БД)
    ├── migrate-auth.yaml              # Auth миграции БД
    └── migrate-transactions.yaml      # Transactions миграции БД
```

**Docker образы:**
- `finwise-auth:latest` - Auth сервис (из `backend/auth/Dockerfile`)
- `finwise-transactions:latest` - Transactions сервис (из `backend/transactions/Dockerfile`)
- `finwise-ml-api:latest` - ML-API сервис (из `backend/ml-api/Dockerfile`)
- `finwise-auth-migrate:latest` - Auth миграции (из `backend/auth/Dockerfile.migrate`)
- `finwise-transactions-migrate:latest` - Transactions миграции (из `backend/transactions/Dockerfile.migrate`)
